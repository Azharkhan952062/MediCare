...
// this function will convert time 12hrs to number of minutes since midnught;
// and return AM : PM;
...
// this function will convert doctor data into a plane text;
...
export async function createDoctor(req, res) {
  try {
    ...
    if (req.file?.path) {
      ...
      imageUrl = uploaded?.secure_url || uploaded?.publicId || imagePublicId;
    }
...
export async function updateDoctor(req, res) {
  try {
    ...
    //if existing then update the image else show a warning;
    if (req.file?.path) {
  ...
  try {
    ...
    if (existing.imagePublicId) {
      try {
        ...
        console.warn("DeleteFromCoudinary warning:", e?.message || e);
      }
    }
...
export async function toggleAvailability(req, res) {
  try {
    ...
    if (typeof doc.availability === "boolean")
      ...
      doc.availability =
        doc.availability === "availability" ? "Unavailable" : "Available";

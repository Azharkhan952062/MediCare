...
export const createAppointment = async (req, res) => {
  try {
    ...
    console.log("CREATE APPOINTMENT USER =", clerkUserId);
    ...
    if (existingBooking) {
      return res.status(409).json({
        success: false,
        message:
          "You already have an appointment with this at the doctor at the selected slot.",
      });
    }
    ...
    if (paymentMethod === "Cash") {
      console.log("ENTERED CASH BLOCK");
      const created = await Appointment.create({
        ...base,
        status: "Pending",
